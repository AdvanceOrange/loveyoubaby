...
/**
 * 改变合作案例
 * @param index 在集合中选取第一个
 */
function changeCooperation(index) {
    console.log(cooperations.length)
    //不是第一次执行,将已经显示的案例重新隐藏
    for(var i = 0; i < 5; i ++) {
        cooperations.eq(oldCooperation[i]).css({"transform":"scale(0,0)"});
    }
    //等待300毫秒，缩放完成后，隐藏元素
    setTimeout(function(){
        for(var i = 0; i < cooperations.length; i ++) {
            cooperations.eq(oldCooperation[i]).css({"display": "none"});
        }
    },500);

    //清空数组
    oldCooperation.clear;
    //等待缩小完成，后执行放大
    setTimeout(function() {
        for (var i = 0; i < 5; i++) {
            //console.log((index + i) % cooperations.length);
            cooperations.eq((index + i) % cooperations.length).css({"display": "block"});
            oldCooperation[i] = (index + i) % cooperations.length;
        }
    },600);
    ...
    setTimeout(function() {
        for(var i = 0; i < cooperations.length; i ++) {
            cooperations.eq(oldCooperation[i]).css({"transform": "scale(1,1)"});
        }
    },700)
}